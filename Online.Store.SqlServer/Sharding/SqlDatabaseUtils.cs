...
namespace Online.Store.SqlServer
{
    ...
    internal static class SqlDatabaseUtils
    {
        ...
        public static bool TryConnectToSqlDatabase()
        {
            ...
            {
                Debug.WriteLine("Failed to connect to SQL database with connection string:");
                Console.WriteLine("\n{0}\n", connectionString);
                Debug.WriteLine("If this connection string is incorrect, please update the Sql Database settings in App.Config.\n\nException message: {0}", e.Message);
                return false;
            }
        }
        ...
        {
            Debug.WriteLine("Creating database {0}", db);
            using (ReliableSqlConnection conn = new ReliableSqlConnection(
                Configuration.GetConnectionString(server, MasterDatabaseName),
                SqlRetryPolicy,
                SqlRetryPolicy))
            {
                ...
                if (engineEdition == 5)
                {
                    ...
                    {
                        Debug.WriteLine("Waiting for database {0} to come online...", db);
                        Thread.Sleep(TimeSpan.FromSeconds(5));
                    }

                    Debug.WriteLine("Database {0} is online", db);
                }
                else
                {
        ...
        {
            Debug.WriteLine("Dropping database {0}", db);
            using (ReliableSqlConnection conn = new ReliableSqlConnection(
                Configuration.GetConnectionString(server, MasterDatabaseName),
                SqlRetryPolicy,
                SqlRetryPolicy))
        ...
        {
            Debug.WriteLine("Executing script {0}", schemaFile);
            using (ReliableSqlConnection conn = new ReliableSqlConnection(
